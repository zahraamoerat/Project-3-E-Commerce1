  ...
  try {
    ...
    const [orders] = await db.execute(
      ...
      SELECT o.order_number AS id, b.business_name AS buyer,
        ...
        o.total_amount AS total, o.order_order_status AS order_status,
    ...
    const [deliveries] = await db.execute(
      ...
      JOIN buyers b ON b.buyer_id = o.buyer_id
      WHERE o.supplier_id = ? ORDER BY d.created_at DESC`,
      [id],
    );
    const [reviews] = await db.execute(
      ...
      WHERE p.supplier_id = ? AND r.status = 'Published' ORDER BY r.review_date DESC`,
      [id, id],
    );
    ...
    const orderStatus = (status) =>
      status === "Shipped" ? "Ready to ship" : status;
...
export async function updateOrder(req, res, next) {
  ...
  try {
    ...
    const [[order]] = await connection.execute(
      "SELECT order_id, order_status AS order_status FROM orders WHERE order_number = ? AND supplier_id = ? FOR UPDATE",
      [req.params.id, id],
    );
    if (!order) {
      ...
      for (const item of items) {
        ...
        if (!stock || Number(stock.quantity) < Number(item.quantity)) {
          ...
          return res
            ...
            .json({
              message:
                "Order cannot be moved to Processing because one or more products do not have enough stock.",
            });
        }
        const previous = Number(stock.quantity),
          next = previous - Number(item.quantity);
    ...
    res.json({
      message:
        willProcess && !wasProcessing
          ? "Order updated and stock deducted."
          : "Order updated.",
    });
  } catch (error) {
    try {
...
export async function updateDelivery(req, res, next) {
  try {
    const statusMap = {
      "Ready for pickup": "Preparing Dispatch",
...
export async function replyToReview(req, res, next) {
  try {
    ...
    res
      .status(201)
      .json({ message: "Reply saved.", reply_id: result.insertId });
  } catch (error) {
    next(error);
  }
}
...
export async function updateProfile(req, res, next) {
  try {
    ...
    if (fields.email !== undefined && !validEmail(fields.email))
      return res
        .status(400)
        .json({ message: "Please enter a valid email address." });
    if (
      fields.businessName !== undefined &&
      String(fields.businessName).trim().length < 2
    )
      return res
        .status(400)
        .json({ message: "Business name must be at least 2 characters." });
    const [city = "", province = ""] = String(
      fields.location ??
        `${supplier[0].city || ""}, ${supplier[0].province || ""}`,
    )
      .split(",")
      .map((value) => value.trim());
    await db.execute(
      "UPDATE suppliers SET business_name = ?, email = ?, phone = ?, city = ?, province = ? WHERE supplier_id = ?",
      [
        fields.businessName || supplier[0].business_name,
        fields.email || supplier[0].email,
