...
async function loadDelivery() {
  ...
  try {
    ...
    if (!buyerId) {
      errorCanRetry.value = false
      errorMessage.value = 'Please sign in to set up this delivery.'
      return
    }
...
.connect-sb-location-setup-spinner {
  ...
  width: 22px;
  height: 22px;
  border: 3px solid rgba(164, 117, 71, 0.25);
...
.connect-sb-location-setup-continue {
  ...
  font-weight: 800;
  cursor: pointer;
  box-shadow: 0 5px 14px rgba(164, 117, 71, 0.25);
  transition:
    transform 180ms ease,
    background 180ms ease;
}
...
.connect-sb-location-setup-pin-destination-body {
  ...
  background: var(--sb-accent);
  transform: rotate(-45deg);
  box-shadow: 0 4px 14px rgba(164, 117, 71, 0.34);
}
...
.connect-sb-location-setup-state-card h2,
.connect-sb-location-setup-map-header h2 { font-family: var(--sb-font-heading); color: var(--sb-ink); }
</style>