...
async function restoreProduct(product) {
  error.value = "";
  try { await fetch(`/api/supplier/products/${product.product_id}/restore`, { method: "POST", headers: { Authorization: `Bearer ${localStorage.getItem("weconnect_token")}` } }); await import("@/data/supplierData").then(m => m.loadSupplierData(true)); } catch (err) { error.value = err.message || "Unable to restore product."; }
}
