...
async function loadDelivery() {
  try {
    ...
    if (!buyerId) {
      throw new Error('Please sign in to track this delivery')
    }

    const response = await fetch(
      `/api/deliveries?buyerId=${encodeURIComponent(buyerId)}`
    )
...
.connect-tracking-status-active .connect-tracking-status-marker span {
  ...
  border-radius: 50%;
  background: var(--sb-accent);
  box-shadow: 0 0 0 5px rgba(164, 117, 71, 0.13);
}
...
.connect-tracking-title, .connect-tracking-section-title h2 { color: var(--sb-ink); }
