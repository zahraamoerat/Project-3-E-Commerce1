  ...
  <div class="supplier_edit_product_edit-page">
    <header class="supplier_edit_product_edit-header">
      ...
      <RouterLink
        ...
        class="supplier_edit_product_ghost-button"
    ...
    <div
      v-else-if="pageError"
      class="supplier_edit_product_empty supplier_edit_product_error"
      role="alert"
    >
      ...
      <button
        type="button"
        class="supplier_edit_product_primary-button supplier_edit_product_empty-action"
        @click="reload"
      >
        Try again
      </button>
    ...
    <form v-else class="supplier_edit_product_edit-grid" @submit.prevent="save">
      <section class="supplier_edit_product_edit-card">
        ...
        <label
          :class="{ supplier_edit_product_invalid: fieldErrors.product_name }"
        >
          Product name
          <input
            id="product-name"
          ...
          />
          <small id="product-name-help" class="supplier_edit_product_hint"
            >Use a clear name buyers will recognise.</small
          >
          <span
            v-if="fieldErrors.product_name"
            id="product-name-error"
            class="supplier_edit_product_field-error"
            >{{ fieldErrors.product_name }}</span
          >
        </label>
        ...
        <div class="supplier_edit_product_two-columns">
          <label
            :class="{
              supplier_edit_product_invalid: fieldErrors.category_name,
            }"
          >
            Category
            <select
              ...
              @change="validateField('category_name')"
            ...
            <span
              v-if="fieldErrors.category_name"
              id="category-error"
              class="supplier_edit_product_field-error"
              >{{ fieldErrors.category_name }}</span
            >
            <span
              v-else-if="categoriesError"
              class="supplier_edit_product_field-error"
              >{{ categoriesError }}</span
            >
          </label>
          ...
          <label :class="{ supplier_edit_product_invalid: fieldErrors.sku }">
            ...
            />
            <small id="sku-help" class="supplier_edit_product_hint"
              >3–40 characters: letters, numbers, dots, underscores or
              hyphens.</small
            >
            <span
              ...
              id="sku-error"
              class="supplier_edit_product_field-error"
              >{{ fieldErrors.sku }}</span
            >
          </label>
        </div>

        <label
          :class="{ supplier_edit_product_invalid: fieldErrors.description }"
        >
          Description
          <textarea
          ...
          <div class="supplier_edit_product_counter-row">
            <small id="description-counter"
              >{{ form.description.length }}/2000 characters</small
            >
            <span
        ...
        <div
          v-if="error"
          class="supplier_edit_product_error supplier_edit_product_inline-error"
          role="alert"
        >
          {{ error }}
        </div>
      ...
      <aside class="supplier_edit_product_edit-side">
        <section
          class="supplier_edit_product_edit-card supplier_edit_product_media-card"
        >
          <div class="supplier_edit_product_section-heading">
            ...
            <span class="supplier_edit_product_media-count"
              >{{ form.images.length }}/8</span
            >
          </div>
          ...
          <div
            ...
            :class="{
              'supplier_edit_product_upload-zone--dragging': isDragging,
              'supplier_edit_product_upload-zone--disabled':
          ...
          >
            <span class="supplier_edit_product_upload-icon" aria-hidden="true"
              >&#8615;</span
            >
            <strong>{{
              form.images.length >= 8
          ...
          <p
            v-if="mediaError"
            class="supplier_edit_product_field-error supplier_edit_product_media-error"
            role="alert"
          >
            {{ mediaError }}
          </p>
          ...
          <div
            ...
            class="supplier_edit_product_thumbnail-strip"
          ...
          >
            <article
              ...
              :class="{
                'supplier_edit_product_thumbnail-tile--primary': index === 0,
              }"
            >
              <img
              ...
              <span v-if="index === 0" class="supplier_edit_product_media-badge"
                >Primary</span
              >
              <span v-else class="supplier_edit_product_media-type">{{
                isNewImage(image) ? "New" : "Saved"
              }}</span>
              ...
              <div class="supplier_edit_product_image-actions">
                ...
                <button
                  type="button"
                  class="supplier_edit_product_remove-image"
          ...
          <p class="supplier_edit_product_media-help">
            The first image is the primary catalog image. Saved images are kept
            unless you remove them; new uploads are marked
            <strong>New</strong> until saved.
          </p>
        </section>
        ...
        <section class="supplier_edit_product_edit-card">
          ...
          <div class="supplier_edit_product_pricing-grid">
            <label
              :class="{ supplier_edit_product_invalid: fieldErrors.price }"
            >
              Selling price
              <div class="supplier_edit_product_input-prefix">
                <span>R</span
                ><input
                  id="product-price"
            ...
            <label
              :class="{
                supplier_edit_product_invalid: fieldErrors.comparePrice,
              }"
            >
              Compare at price
              <div class="supplier_edit_product_input-prefix">
                ...
                ><input
                  ...
                  min="0"
                  step="0.01"
                  aria-describedby="compare-price-error"
                  @blur="validateField('comparePrice')"
                />
              </div>
              <span
                v-if="fieldErrors.comparePrice"
                id="compare-price-error"
                class="supplier_edit_product_field-error"
                >{{ fieldErrors.comparePrice }}</span
              >
            </label>
          </div>
          ...
          <div class="supplier_edit_product_pricing-grid">
            <label
              :class="{ supplier_edit_product_invalid: fieldErrors.quantity }"
            >
              Stock quantity
              <input
                id="product-quantity"
            ...
            <label
              :class="{
                supplier_edit_product_invalid: fieldErrors.low_stock_threshold,
              }"
            >
              Low-stock alert
              <input
                ...
                @blur="validateField('low_stock_threshold')"
              />
              <span
                v-if="fieldErrors.low_stock_threshold"
                id="threshold-error"
                class="supplier_edit_product_field-error"
                >{{ fieldErrors.low_stock_threshold }}</span
              >
            </label>
          </div>

          <div
            class="supplier_edit_product_stock-callout"
            :class="`supplier_edit_product_stock-callout--${stockStatusClass}`"
          >
            <div>
            ...
            <span
              class="supplier_edit_product_stock-dot"
              aria-hidden="true"
            ></span>
          </div>
      ...
      <div class="supplier_edit_product_action-bar">
        <span v-if="dirty" class="supplier_edit_product_unsaved" role="status"
          >Unsaved changes</span
        >
        <span v-else class="supplier_edit_product_saved-state"
          >No unsaved changes</span
        >
        <div class="supplier_edit_product_action-buttons">
          <button
            type="button"
            class="supplier_edit_product_text-button"
            :disabled="saving"
            @click="cancelEdit"
          >
            Cancel
          ...
          <button
            class="supplier_edit_product_primary-button supplier_edit_product_save-button"
            type="submit"
      ...
      <p
        v-if="message"
        class="supplier_edit_product_success"
        role="status"
        aria-live="polite"
      >
        {{ message }}
...
const product = computed(
  () =>
    products.value.find((item) => item.product_id === productId.value) || null,
);
const dirty = computed(
  () => JSON.stringify(snapshot(form)) !== JSON.stringify(original.value),
);
...
const stockStatusClass = computed(() =>
  reactiveStockStatus.value.toLowerCase().replaceAll(" ", "-"),
);
...
onMounted(async () => {
  try {
    ...
    if (!product.value && !supplierError.value)
      pageError.value = "The product could not be found.";
...
function validateField(field) {
  ...
  if (field === "product_name" && String(value || "").trim().length < 3)
    result = "Product name must be at least 3 characters.";
  if (field === "category_name" && !value)
    result = "Select a product category.";
  if (
    field === "sku" &&
    value &&
    !/^[A-Za-z0-9][A-Za-z0-9._-]{2,39}$/.test(String(value).trim())
  )
    result =
      "SKU must be 3–40 characters using letters, numbers, dots, underscores or hyphens.";
  if (field === "description" && String(value || "").length > 2000)
    result = "Description must be 2000 characters or fewer.";
  if (
    field === "price" &&
  ...
  if (
    ...
    value !== null &&
    value !== "" &&
    (!Number.isFinite(Number(value)) || Number(value) < Number(form.price))
  )
    result =
  ...
  if (
    ...
    result = "Quantity must be a non-negative whole number.";
  if (
    field === "low_stock_threshold" &&
    (!Number.isInteger(Number(value)) || Number(value) < 0)
  )
    result = "Low-stock threshold must be a non-negative whole number.";
...
function cancelEdit() {
  if (!dirty.value || window.confirm("Discard your unsaved changes?"))
...
.supplier_edit_product_edit-page {
  min-height: 100vh;
  padding: 32px clamp(18px, 4vw, 48px) 120px;
  background: #f7f4f1;
  color: #4d3933;
...
.supplier_edit_product_edit-header h1 {
  ...
  color: #3f2d27;
  font:
    700 clamp(28px, 3vw, 36px)/1.15 Georgia,
    "Times New Roman",
    serif;
}
...
.supplier_edit_product_ghost-button {
  ...
  box-shadow: 0 2px 8px rgba(63, 45, 39, 0.04);
...
.supplier_edit_product_edit-card {
  ...
  border-radius: 14px;
  background: #fff;
  box-shadow: 0 8px 24px rgba(63, 45, 39, 0.06);
}
.supplier_edit_product_edit-card h2 {
  margin: 8px 0 24px;
  color: #44322c;
  font:
    700 21px/1.25 Georgia,
    "Times New Roman",
    serif;
}
...
.supplier_edit_product_edit-card textarea:focus {
  border-color: #c77a4e;
  background: #fff;
  box-shadow: 0 0 0 3px rgba(199, 122, 78, 0.12);
}
...
.supplier_edit_product_pricing-grid {
  display: grid;
  grid-template-columns: 1fr 1fr;
  gap: 16px;
}
.supplier_edit_product_hint {
  color: #9a887f;
  font-size: 10px;
  font-weight: 400;
  line-height: 1.4;
...
.supplier_edit_product_invalid input,
.supplier_edit_product_invalid select,
.supplier_edit_product_invalid textarea {
  border-color: #d88980;
  background: #fff8f7;
}
.supplier_edit_product_counter-row {
  display: flex;
  justify-content: space-between;
  gap: 12px;
  min-height: 16px;
...
.supplier_edit_product_section-heading {
  display: flex;
  justify-content: space-between;
  align-items: flex-start;
  gap: 16px;
}
.supplier_edit_product_section-heading h2 {
  margin-bottom: 15px;
}
.supplier_edit_product_media-count {
  padding: 5px 8px;
  border-radius: 999px;
  background: #f5e3d7;
  color: #8c573f;
...
.supplier_edit_product_upload-zone--dragging {
  border-color: #c87548;
  background: #fff2e9;
  transform: translateY(-1px);
}
...
.supplier_edit_product_upload-zone:focus-visible {
  outline: 3px solid rgba(199, 122, 78, 0.18);
  outline-offset: 2px;
}
...
.supplier_edit_product_media-error {
  display: block;
  margin-top: 10px;
}
...
.supplier_edit_product_thumbnail-tile--primary {
  border-color: #d2763d;
}
...
.supplier_edit_product_media-type {
  position: absolute;
  left: 6px;
  bottom: 6px;
  padding: 4px 6px;
...
.supplier_edit_product_image-actions {
  position: absolute;
  left: 5px;
  right: 5px;
  top: 5px;
...
.supplier_edit_product_image-actions button {
  width: 25px;
  height: 25px;
  display: grid;
  place-items: center;
...
.supplier_edit_product_image-actions button:hover {
  background: #fff2e9;
}
.supplier_edit_product_image-actions .supplier_edit_product_remove-image {
  margin-left: auto;
  color: #a8473d;
}
...
.supplier_edit_product_input-prefix {
  display: flex;
  align-items: center;
  border: 1px solid #ded5cf;
  border-radius: 8px;
...
.supplier_edit_product_input-prefix:focus-within {
  border-color: #c77a4e;
  box-shadow: 0 0 0 3px rgba(199, 122, 78, 0.12);
  background: #fff;
}
.supplier_edit_product_input-prefix span {
  padding-left: 13px;
  color: #9a887f;
  font-weight: 800;
}
.supplier_edit_product_input-prefix input {
  border: 0;
  box-shadow: none;
...
.supplier_edit_product_stock-callout div {
  display: flex;
  flex-direction: column;
  gap: 3px;
}
...
.supplier_edit_product_stock-callout--in-stock {
  border-color: #cfe2d2;
  background: #f1f8f2;
  color: #397044;
}
.supplier_edit_product_stock-callout--low-stock {
  border-color: #ead7b0;
  background: #fff9ed;
  color: #8a6728;
}
.supplier_edit_product_stock-callout--out-of-stock {
  border-color: #efc8c3;
  background: #fff4f2;
  color: #a8473d;
}
.supplier_edit_product_stock-dot {
  width: 10px;
  height: 10px;
  border-radius: 50%;
  background: currentColor;
}
.supplier_edit_product_last-updated {
  display: flex;
  justify-content: space-between;
  gap: 16px;
  padding-top: 12px;
...
.supplier_edit_product_last-updated strong {
  color: #70564b;
}
...
.supplier_edit_product_inline-error {
  margin-top: 8px;
  padding: 11px 13px;
  border-radius: 8px;
  font-size: 12px;
...
.supplier_edit_product_empty {
  ...
  background: #fff;
  text-align: center;
  box-shadow: 0 8px 24px rgba(63, 45, 39, 0.05);
}
...
.supplier_edit_product_empty p {
  color: #87756d;
  font-size: 13px;
}
.supplier_edit_product_empty-action {
  max-width: 220px;
  margin: 14px auto 0;
  text-decoration: none;
}
.supplier_edit_product_spinner {
  width: 34px;
  height: 34px;
  margin: 0 auto;
  border: 3px solid #eaded5;
  border-top-color: #d2763d;
  border-radius: 50%;
  animation: supplierEditSpin 0.8s linear infinite;
}
@keyframes supplierEditSpin {
  to {
    transform: rotate(360deg);
  }
}
.supplier_edit_product_action-bar {
  position: sticky;
  bottom: 16px;
  grid-column: 1/-1;
  z-index: 10;
  ...
  box-shadow: 0 10px 30px rgba(63, 45, 39, 0.14);
  backdrop-filter: blur(8px);
}
.supplier_edit_product_unsaved,
.supplier_edit_product_saved-state {
  font-size: 11px;
  font-weight: 800;
}
.supplier_edit_product_unsaved {
...
.supplier_edit_product_action-buttons {
  display: flex;
  gap: 9px;
  min-width: 260px;
}
...
.supplier_edit_product_primary-button {
  ...
  background: #d2763d;
  color: #fff;
  box-shadow: 0 5px 12px rgba(180, 91, 43, 0.18);
  text-decoration: none;
}
...
.supplier_edit_product_save-button {
  flex: 1;
}
.supplier_edit_product_text-button {
  flex: 0 0 100px;
...
.supplier_edit_product_text-button:hover:not(:disabled) {
  background: #f7f0ec;
}
