  ...
  <div class="shop-page">
    ...
    <section class="shop-hero">
      ...
      <div class="hero-heading">
        <p class="shop-eyebrow">EXPLORE ALL THE LISTED PRODUCTS AND STORES.</p>
        <h1>Shop</h1>
      </div>
    </section>
...
function persistBasket() {
  localStorage.setItem(basketKey(), JSON.stringify(basket.value))
  setBasketCount(countBasketItems(basket.value))
}
function persistWishlist() { localStorage.setItem(wishlistKey(), JSON.stringify(savedProducts.value)) }
...
async function syncBasketFromCart() {
  try {
    ...
    // Keep any locally tracked items the cart service didn't return so the
    // user's additions are never lost.
    for (const [productId, entry] of Object.entries(basket.value)) {
      if (!(productId in next)) next[productId] = entry
    }
    basket.value = next
    persistBasket()
  } catch {
...
async function toggleBasket(product) {
  ...
  try {
    ...
    // Keep the item in the local basket so it still shows up in the cart
    // even when the cart service cannot be reached.
    basket.value = {
      ...basket.value,
      [product.id]: {
        product,
...
async function addDetailToBasket({ goToCart = false } = {}) {
  ...
  try {
    ...
    if (cartItemId) {
      ...
      const created = await cartRequest('', {
        method: 'POST',
        body: JSON.stringify({ buyerId: buyerId.value, productId: product.id, quantity: nextQuantity })
      })
      cartItemId = created.cartItemId
    }
...
.filter-heading h2 { margin: 0; color: #403c39; font-family: Georgia, "Times New Roman", serif; font-size: 15px; }
...
.shop-hero { width: min(1280px, 94%); margin: 0 auto; display: grid; justify-items: start; align-items: center; gap: 7px; padding: 26px 0 20px; min-height: 118px; background: transparent; color: #4E342E; }
.shop-hero h1 { order: 2; margin: 0; font-family: Georgia, "Times New Roman", serif; color: #4E342E; font-size: 42px; font-weight: 600; line-height: 1.05; }
.shop-hero p { order: 1; margin: 0; color: #c48b5b; font-size: 11px; font-weight: 800; letter-spacing: 1.7px; }
...
@media (max-width: 760px) { .shop-main, .shop-hero { width: calc(100% - 24px); } }
