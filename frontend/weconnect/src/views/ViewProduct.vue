      ...
      <div class="detail-layout">
        ...
        <section class="product-info">
          ...
          <div class="detail-block">
            <strong class="block-label">Finish <span class="selection-note">{{ selectedFinishLabel }}</span></strong>
          ...
          <div class="detail-block">
            <strong class="block-label">Pack size <span class="selection-note">{{ packSizes[selectedSize] }}</span><a href="#" @click.prevent="showSizeGuide">View size guide</a></strong>
          ...
          <div class="quantity-row"><div class="quantity-stepper"><button type="button" aria-label="Decrease quantity" :disabled="quantity <= 1" @click="quantity = Math.max(1, quantity - 1)">&#8722;</button><span>{{ quantity }}</span><button type="button" aria-label="Increase quantity" :disabled="quantity >= product.quantity" @click="quantity = Math.min(product.quantity, quantity + 1)">+</button></div><span class="stock-note">{{ product.quantity }} units in stock</span></div>
          <div class="buy-row"><button type="button" class="btn btn-primary" :disabled="!product.quantity" @click="addToOrder"><span aria-hidden="true">+</span> Add to order</button><button type="button" class="btn btn-dark" :disabled="!product.quantity" @click="orderNow">Order now</button><button type="button" class="heart-button" :class="{ saved: isSaved }" aria-label="Save for later" :aria-pressed="isSaved" @click="toggleSaved">{{ isSaved ? '&#9829;' : '&#9825;' }}</button></div>

          <div class="meta"><div><b>SKU</b> {{ product.sku || 'Not provided' }}</div><div><b>Tags</b> {{ product.category_name }}, Wholesale, Bulk</div><div class="share-row"><b>Share</b><button type="button" aria-label="Share on Facebook" @click="shareProduct('Facebook')">f</button><button type="button" aria-label="Share on X" @click="shareProduct('X')">x</button><button type="button" aria-label="Share on Pinterest" @click="shareProduct('Pinterest')">p</button></div></div>
        </section>
      </div>
...
function showSizeGuide() {
  Swal.fire({
    ...
    html: `<div class="sweet-size-guide"><p><b>25 pack</b><span>Trial quantity for smaller orders</span></p><p><b>50 pack</b><span>Popular for regular restocks</span></p><p><b>100+ pack</b><span>Best value for wholesale buyers</span></p></div>`,
...
.quantity-row { display: flex; flex-wrap: wrap; align-items: center; gap: 16px; margin-bottom: 18px; }.quantity-stepper { display: flex; align-items: center; overflow: hidden; border: 1px solid #e7e0d8; border-radius: 9px; }.quantity-stepper button { width: 34px; height: 36px; border: 0; background: #fff; color: #3c2b26; font-size: 16px; cursor: pointer; }.quantity-stepper button:disabled, .btn:disabled { cursor: not-allowed; opacity: .45; }.quantity-stepper span { width: 38px; text-align: center; font-size: 14px; font-weight: 600; }.stock-note { color: #3f7a53; font-size: 12.5px; font-weight: 600; }.buy-row { display: flex; flex-wrap: wrap; align-items: center; gap: 10px; margin-bottom: 10px; }.btn { display: inline-flex; align-items: center; justify-content: center; gap: 7px; padding: 12px 22px; border: 1px solid transparent; border-radius: 9px; font-size: 13.5px; font-weight: 600; cursor: pointer; }.btn-primary { background: #e0793c; color: #fff; }.btn-dark { border-color: #523a33; background: #523a33; color: #fff; }.heart-button { width: 44px; height: 44px; border: 1px solid #e7e0d8; border-radius: 9px; background: #fff; color: #6b5d56; font-size: 20px; cursor: pointer; }.heart-button.saved { border-color: #e0793c; background: #fff3eb; color: #c9631f; }.meta { display: flex; flex-direction: column; gap: 7px; padding-top: 14px; border-top: 1px solid #f0ebe4; color: #6b5d56; font-size: 12.5px; }.meta b { margin-right: 6px; color: #3c2b26; }.selection-note { margin-left: 5px; color: #c9631f; font-weight: 400; }.share-row { display: flex; align-items: center; gap: 10px; }.share-row button { display: inline-flex; align-items: center; justify-content: center; width: 28px; height: 28px; border: 0; border-radius: 7px; background: #f0ebe4; color: #6b5d56; cursor: pointer; }.share-row button:hover { background: #ead9ce; }
