<template>
  <div class="supplier_reviews_reviews">
    <header class="supplier_reviews_header">
      ...
      <div class="supplier_reviews_rating"><strong>{{ averageRating.toFixed(1) }}</strong><span>{{ "★".repeat(Math.round(averageRating)) }}{{ "☆".repeat(5 - Math.round(averageRating)) }}</span><small>Average supplier rating</small></div>
    </header>
    <section class="supplier_reviews_summary">
      <article><span>Reviews</span><strong>{{ reviews.length }}</strong></article>
...
const { reviews, replyToReview } = useSupplierData(); const averageRating = computed(() => reviews.value.length ? reviews.value.reduce((sum,r)=>sum+Number(r.rating||0),0)/reviews.value.length : 0); const responseRate = computed(() => reviews.value.length ? Math.round((reviews.value.filter(r=>r.replied).length/reviews.value.length)*100) : 0); const onlyUnanswered = ref(false); const message = ref(""); const unanswered = computed(() => reviews.value.filter((review) => !review.replied).length); const visibleReviews = computed(() => onlyUnanswered.value ? reviews.value.filter((review) => !review.replied) : reviews.value);
