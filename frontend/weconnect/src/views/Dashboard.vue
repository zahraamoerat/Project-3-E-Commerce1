<template>
  <div class="supplier_dashboard_dashboard">
    ...
    <section class="supplier_dashboard_stats">
      <article><span>Catalog products</span><strong>{{ productCount }}</strong><small>Live from your catalog</small></article>
      <article><span>Orders this month</span><strong>{{ orders.length }}</strong><small>Live supplier orders</small>
      </article>
      <article><span>Open revenue</span><strong>{{ money(revenue) }}</strong><small>Current order value</small>
      </article>
