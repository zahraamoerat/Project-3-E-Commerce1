    ...
    <section class="stats-grid">
      ...
      <div class="card stat-card highlight">
        ...
        <span class="stat-trend warning"
          >{{ stats.pendingInvoices }} invoice due</span
        >
      </div>
      <div class="card stat-card">
      ...
      <div class="card stat-card">
        ...
        <span class="stat-trend positive"
          >+{{ stats.spendChangePercent }}% vs last month</span
        >
      </div>
    </section>
      ...
      <section class="side-col">
        <div class="card">
          ...
          <div class="tracking-steps">
            ...
            <span :class="{ active: trackedOrder.status !== 'awaiting_pickup' }"
              >Dispatched</span
            >
            <span
              :class="{
              ...
              >In transit</span
            >
            <span :class="{ active: trackedOrder.status === 'delivered' }"
              >Delivered</span
            >
          </div>
          <div
          ...
          >
            ...
            <div v-if="trackedOrder.deliveryId" class="dashboard-map-action">
              View live tracking →
            </div>
          </div>
        </div>
...
const recentOrders = ref([
  {
    ...
    itemsSummary: "Maize seed (10kg bags)...",
    status: "out_for_delivery",
    eta: "24 min",
  },
  {
    orderId: "SB-1041",
    supplierName: "Karoo Fertiliser T...",
    itemsSummary: "NPK fertiliser (50kg)...",
    status: "dispatched",
    eta: "Tomorrow 9–11am",
  },
  {
    orderId: "SB-1038",
    supplierName: "CropGuard Distrib...",
    itemsSummary: "Crop protection spray...",
    status: "delivered",
    eta: "—",
  },
  {
    orderId: "SB-1035",
    supplierName: "FarmTech Equipm...",
    itemsSummary: "Irrigation pipe fittings...",
    status: "processing",
...
const suggestedSuppliers = ref([
  {
    supplierId: 1,
    companyName: "Highveld Seed Co.",
    description: "Certified maize, wheat & soya seed supplier",
  },
  {
    supplierId: 2,
    companyName: "Karoo Fertiliser Traders",
    description: "Bulk NPK, lime, and soil conditioner supply",
  },
  {
    supplierId: 3,
    companyName: "FarmTech Equipment Parts",
    description: "Tractor and irrigation equipment spares",
  },
]);
...
const notifications = ref([
  {
    notificationId: 1,
    type: "order_update",
    message: "Karoo Fertiliser Traders has accepted your NPK fertiliser order",
    isRead: false,
  },
  {
    notificationId: 2,
    type: "payment_reminder",
    message: "Invoice for #SB-1035 is due in 2 days (R1,450)",
    isRead: false,
  },
  {
    notificationId: 3,
    type: "rating_request",
    message: "Share your feedback for FarmTech Equipment Parts' delivery",
    isRead: true,
  },
]);
...
function mapDeliveryStatus(status) {
  ...
  if (["in transit", "out for delivery", "delayed"].includes(normalized))
    return "in_transit";
...
async function loadDashboard() {
  try {
      ...
      orderList.slice(0, 4).map(async (order) => {
        ...
        try {
          const items = await fetchJson(
            `/orders?buyerId=1/${order.order_id}/items`,
          );
          if (Array.isArray(items) && items.length) {
            itemsSummary = items
              .map(
                (item) => `${item.product_name || "Product"} ×${item.quantity}`,
              )
              .join(", ");
          }
        } catch {
    ...
    console.warn(
      "Dashboard API unavailable, showing sample data:",
      error.message,
    );
  }
}
...
.notifications-list li.unread strong {
  color: var(--color-text);
}
.dot {
  width: 8px;
